...
import { Home, Compass, Heart, User, LogOut, Sparkles, PenTool } from "lucide-react";
...
export function Sidebar() {
  ...
  const planNames: Record<string, string> = { basic: "Básico", pro: "Pro", premium: "Premium" };

  return (
    <div className="hidden md:flex w-64 flex-col fixed inset-y-0 z-50 border-r bg-card/90 backdrop-blur-xl border-card-border">
      <div className="p-6 flex items-center gap-3">
        <Brand />
        <div className="hidden relative flex items-center justify-center w-10 h-10 bg-primary/20 rounded-lg border border-primary/50 text-primary">
        ...
        <div className="flex flex-col">
           <span className="font-bold text-lg leading-tight">AURA</span>
           <span className="font-bold text-cyan-300 text-sm leading-tight">IA</span>
        </div>
      </div>

      <div className="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
        <div className="mb-4">
          <p className="px-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Navegação</p>
          <div className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                  <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${isActive ? 'bg-gradient-to-r from-violet-500/20 to-cyan-400/10 text-foreground font-medium shadow-[inset_0_0_18px_hsl(271_91%_63%_/_0.12)]' : 'text-muted-foreground hover:bg-secondary/70 hover:text-foreground'}`}>
        ...
        <div className="mb-4 pt-4 border-t border-border/50">
          ...
          <div className="space-y-1">
            <Link href="/generate" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${location === '/generate' ? 'bg-gradient-to-r from-violet-500/20 to-cyan-400/10 text-foreground font-medium' : 'text-muted-foreground hover:bg-secondary/70 hover:text-foreground'}`}>
              <Sparkles size={17} className="text-primary" />
              Gerar prompt com IA
            </Link>
            <Link href="/my-prompts" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${location === '/my-prompts' || location.startsWith('/my-prompts/') ? 'bg-gradient-to-r from-violet-500/20 to-cyan-400/10 text-foreground font-medium' : 'text-muted-foreground hover:bg-secondary/70 hover:text-foreground'}`}>
              <PenTool size={17} />
              Meus Prompts
            </Link>
          </div>
        ...
        <div className="p-4 border-t border-border/50">
          <div className="flex items-center gap-3 px-2 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">Plano {planNames[user.plan] || user.plan}</p>
            </div>
          </div>
...
export function BottomNav() {
  ...
  const GenerateIcon: React.ComponentType<{ size?: number; className?: string }> = () => (
    <div className="w-10 h-10 -mt-5 rounded-full bg-gradient-to-br from-violet-500 to-cyan-400 flex items-center justify-center text-white shadow-[0_0_18px_hsl(271_91%_63%_/_0.55)]">
      <Sparkles size={18} />
    </div>
  );
  ...
  const navItems: Array<{
    ...
    { href: "/dashboard", label: "Início", icon: Home },
    { href: "/explore", label: "Explorar", icon: Compass },
    { href: "/generate", label: "Gerar", icon: GenerateIcon },
    { href: "/favorites", label: "Favoritos", icon: Heart },
    { href: "/profile", label: "Perfil", icon: User },
  ];

  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-xl border-t border-card-border pb-safe">
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-foreground flex">
      <Sidebar />
      <div className="flex-1 flex flex-col md:ml-64 min-w-0 pb-[calc(4rem+env(safe-area-inset-bottom))] md:pb-0">
        {children}
      </div>
      <BottomNav />
    </div>
